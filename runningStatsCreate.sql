...
GO
CREATE PROCEDURE [dbo].[runningStats]
    @startDate DATE,
    @endDate DATE,
...
AS
    ...
    if (@activity = 'cycling')
    BEGIN        
        SELECT ROUND(AVG(cast(RIGHT(cast(Avg_Pace as nvarchar(5)), 4)as float)),1) as Avg_MPH, MAX(Max_HR) as Max_HR, RIGHT(MAX(Avg_Pace), 4) as Max_MPH, MAX(Distance) as MaxDistance, ROUND(AVG(Distance),1) as AvgDistance, AVG(Calories) AS AvgCalories 
        FROM Activities4 where date >= @startDate AND date <= @endDate AND Activity = @activity;
    END
